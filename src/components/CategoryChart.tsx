...
const CategoryChart: React.FC = () => {
  const [categoryData, setCategoryData] = useState<TimeSeriesData[]>([]);
  ...
  const csvFilePath = '/data/capital_one_category.csv';
  ...
  const fetchCategoryData = async () => {
    try {
      ...
      const categoryRaw = await loadCSV(csvFilePath);
      ...
      const transformedCategoryData = transformTimeSeriesData(categoryRaw, 'Category Level 8', 'Spend_USD');
      
      // Filtrar fechas para asegurarse de que no haya fechas futuras
      const today = new Date();
      const filteredData = transformedCategoryData.filter(item => {
        try {
      ...
      const categories = getUniqueCategories(categoryRaw, 'Category Level 8');
  ...
  const handleCategoryToggle = (category: CategoryOption) => {
      ...
      {selectorOpen && (
        <div className="brand-dropdown-menu">
          <div className="brand-dropdown-header">
            <div className="brand-dropdown-title">Select categories</div>
  ...
  if (categoryData.length === 0 || selectedCategories.length === 0) {
    return (
      <ChartCard title="Ad Spend by Category" filter={categoryFilter}>
