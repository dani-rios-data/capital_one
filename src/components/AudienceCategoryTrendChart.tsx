...
import { 
  loadCSV, COLORS, AUDIENCE_COLORS,
...
// Componente del selector para las categorías de audiencia
...
const AudienceCategoryTrendChart: React.FC = () => {
  ...
  const fetchAudienceData = async () => {
    try {
      ...
      // Seleccionar las categorías con mayor gasto por defecto si no hay ninguna seleccionada
      if (categories.length > 0 && selectedCategories.length === 0) {
        selectTopCategories(filteredData, categories);
      }
                ...
                <div className="brand-checkbox">
                  ...
                  >
                    <span 
                      ...
                      style={{ 
                        backgroundColor: AUDIENCE_COLORS[category.value] || COLORS[categoryOptions.findIndex(c => c.value === category.value) % COLORS.length] 
                      }}
                    ></span>
                    {category.label}
                  </label>
...
export default AudienceCategoryTrendChart; 